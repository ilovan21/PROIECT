...
public class GameBoard {
	...
	private clone.Leaderboards lBoard;
	...
	public GameBoard(int x, int y) {
		...
		lBoard = clone.Leaderboards.getInstance();
		lBoard.loadScores();
	...
	private void checkKeys() {
		if (!clone.Keys.pressed[KeyEvent.VK_LEFT] && clone.Keys.prev[KeyEvent.VK_LEFT]) {
			moveTiles(LEFT);
			if (!hasStarted) hasStarted = !dead;
		}
		if (!clone.Keys.pressed[KeyEvent.VK_RIGHT] && clone.Keys.prev[KeyEvent.VK_RIGHT]) {
			moveTiles(RIGHT);
			if (!hasStarted) hasStarted = !dead;
		}
		if (!clone.Keys.pressed[KeyEvent.VK_UP] && clone.Keys.prev[KeyEvent.VK_UP]) {
			moveTiles(UP);
			if (!hasStarted) hasStarted = !dead;
		}
		if (!clone.Keys.pressed[KeyEvent.VK_DOWN] && clone.Keys.prev[KeyEvent.VK_DOWN]) {
			moveTiles(DOWN);
			if (!hasStarted) hasStarted = !dead;
		}
	}
