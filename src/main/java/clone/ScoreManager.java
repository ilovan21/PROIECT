...
public class ScoreManager {
	...
	private String temp;
	private clone.GameBoard gBoard;

	// Random booleans
	private boolean newGame;

	public ScoreManager(clone.GameBoard gBoard) {
		try {
	...
	private void createFile() {
		...
		try {
			...
			writer.write("" + 0);
			writer.newLine();
			for (int row = 0; row < clone.GameBoard.ROWS; row++) {
				for (int col = 0; col < clone.GameBoard.COLS; col++) {
					if(row == clone.GameBoard.ROWS - 1 && col == clone.GameBoard.COLS - 1){
						writer.write("" + 0);
					}
					else{
						writer.write(0 + "-");
	...
	public void saveGame() {
		...
		try {
			...
			for (int row = 0; row < clone.GameBoard.ROWS; row++) {
				for (int col = 0; col < clone.GameBoard.COLS; col++) {
					this.board[row * clone.GameBoard.COLS + col] = gBoard.getBoard()[row][col] != null ? gBoard.getBoard()[row][col].getValue() : 0;
					if (row == clone.GameBoard.ROWS - 1 && col == clone.GameBoard.COLS - 1)
						writer.write("" + board[row * clone.GameBoard.COLS + col]);
					else writer.write(board[row * clone.GameBoard.COLS + col] + "-");
				}
			}
			writer.close();
		} catch (Exception e) {
